...
import api, { bannersAPI, API_BASE_URL } from "../../services/api";
...
const FullAdminBanners = () => {
  ...
  const { data: banners, isLoading } = useQuery("banners", bannersAPI.getAll, {
    select: (response) => response.data || [],
  });

  const deleteMutation = useMutation((id) => bannersAPI.delete(id), {
    onSuccess: () => {
...
const BannerFormModal = ({ banner, onClose, onSuccess }) => {
  ...
  const saveMutation = useMutation(
    (data) =>
      banner ? bannersAPI.update(banner.id, data) : bannersAPI.create(data),
    {
      onSuccess: () => {
        toast.success(
          banner
