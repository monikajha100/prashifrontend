...
const AdminPromotionalBanners = () => {
  ...
  const fetchBanners = async () => {
    try {
      ...
      const response = await api.get("/admin/promotional-banners/admin");
  ...
  const handleDelete = async (id) => {
    if (
      ...
      try {
        await api.delete(`/admin/promotional-banners/${id}`);
        fetchBanners();
      } catch (error) {
  ...
  const handleToggleActive = async (id) => {
    try {
      await api.patch(`/admin/promotional-banners/${id}/toggle`);
      fetchBanners();
    } catch (error) {
