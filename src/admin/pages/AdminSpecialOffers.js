...
const AdminSpecialOffers = () => {
  ...
  const fetchOffers = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/special-offers/admin/all`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingOffer) {
        await axios.put(`${API_BASE_URL}/special-offers/${editingOffer.id}`, formData, config);
        toast.success('Offer updated successfully!');
      } else {
        await axios.post(`${API_BASE_URL}/special-offers`, formData, config);
  ...
  const handleToggleActive = async (id) => {
    try {
      const token = localStorage.getItem('token') || localStorage.getItem('adminToken');
      await axios.patch(`${API_BASE_URL}/special-offers/${id}/toggle`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
                ...
                {formData.timer_enabled && (
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e) => setFormData({ ...formData, timer_text: e.target.value })}
          ...
          offers.map(offer => (
            <div key={offer.id} className="offer-card">
              ...
              {offer.timer_enabled && offer.timer_text && (
                <div className="offer-detail timer">
                  {offer.timer_text}
                </div>
              )}
              ...
              {(offer.start_date || offer.end_date) && (
                <div className="offer-dates">
                  {offer.start_date && <span>From: {new Date(offer.start_date).toLocaleDateString()}</span>}
              ...
              <div className="offer-actions">
                ...
                <button 
                  className={`btn-toggle ${offer.is_active ? 'deactivate' : 'activate'}`}
